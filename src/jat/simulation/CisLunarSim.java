	...
	protected void filter()
	{
		...
		{			
			if(simTime.get_sim_time()%(created_meas.frequency[i]) ==0 )
			{
					...
					if(created_meas.measurementTypes[i].equals("y_angle_los")){
...
//			Matrix Covariance = EKF.pold;
			...
			VectorN rTrue, vTrue;
			rTrue = trueState.get(0,3);
			vTrue = trueState.get(3,3);
			Matrix T = RSW_Frame.ECI2RIC(rTrue,vTrue);
			
			VectorN err = T.times(ErrState.get(0,3));
			RSW_Frame RSW = new RSW_Frame(rTrue,vTrue);
			VectorN VRIC = RSW.transform(ErrState.get(0,3),ErrState.get(3,3));
			//VectorN VRIC = RSW.transformVelocity(TrueRic, ErrState.get(3,3));
			VectorN stateOut2 = new VectorN(vecTime,VRIC);
			
			
			new PrintStream(RICError[numSats]).println(stateOut2.toString());
			...
			Matrix Tfull = new Matrix(6,6);
			Tfull.setMatrix(0,0,T);
			Tfull.setMatrix(3,3,T);
			
			Matrix TransMat = (Tfull.times(Covariance.getMatrix(0,5,0,5)));
			Matrix RICCov   = TransMat.times(Tfull.transpose());
			
			
			double[] tmp = new double[6];
			for(int j = 0; j < 6; j++)
			{
				//tmp[j] = Covariance.get(j,j);
				tmp[j] = RICCov.get(j,j);
			}
