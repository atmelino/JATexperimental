...
/**
 * <P>
 * FourRWManeuver contains the equations of motion for the simulation of a rigid
 * spacecraft performing attitude reorientation maneuver using 4 reaction wheels
 * in a pyramid configuration. The class implements
 * jat.alg.integrators.Derivatives and jat.alg.integrateors.Printble, so an
 * outside application code can perform a numerical simulation of equations of
 * motion defined in this class and get the output from the simulation.
 * 
 * @author Noriko Takada Modification since the last version Switched to the
 *         interface EquationsOfMotion
 */
...
public class FourRWManeuver implements EquationsOfMotion {
	 ...
	 * @param quat_values
	 *            [][] Two dimensional array that contains quarternions from
	 *            simulation
	 * @param I1
	 *            I2 I3 Principle moments of inertia about 1, 2, and 3 axis
	 *            respectively
	 * @param J
	 *            Moment of inertia of reaction wheel
	 * @param angle
	 *            Orientation angle of reaction wheel
	 * @param psi
	 *            Target euler angle
	 * @param phi
	 *            Target euler angle
	 ...
	 *            Target phi angle
	 */
	public FourRWManeuver(double time_step, double a, double b, double c,
			float quat_values[][]) {
		setupPlots();
		this.time_step = time_step;
		this.quat_values = quat_values;
	 ...
	 */
	void setupPlots() {
		// FourPlots wheel_velocity_plot = new FourPlots(); // ThreePlots
		// angullar_velocity_plot = new ThreePlots();
	...
	/**
	 * Compute the derivatives. Equations of Motion
	 * 
	...
	public double[] derivs(double t, double[] x) {
		// Error quarternions (Necessary to determine the required control
		// torque)
		...
		if (number_of_RW == FOUR_RW) {
			...
			double p1 = J * x[7] * (b3 * x[1] - b2 * x[2]) + J * x[8]
					* (g3 * x[1] - g2 * x[2]) + J * x[9]
					* (a3 * x[1] - a2 * x[2]) + J * x[10]
					* (d3 * x[1] - d2 * x[2]);
			double p2 = J * x[7] * (b1 * x[2] - b3 * x[0]) + J * x[8]
					* (g1 * x[2] - g3 * x[0]) + J * x[9]
					* (a1 * x[2] - a3 * x[0]) + J * x[10]
					* (d1 * x[2] - d3 * x[0]);
			double p3 = J * x[7] * (b2 * x[0] - b1 * x[1]) + J * x[8]
					* (g2 * x[0] - g1 * x[1]) + J * x[9]
					* (a2 * x[0] - a1 * x[1]) + J * x[10]
					* (d2 * x[0] - d1 * x[1]);
			...
			// Q matrix
			Matrix pseudoQ_matrix = takePseudoInverse(J, b1, b2, b3, g1, g2,
					g3, a1, a2, a3, d1, d2, d3);
			...
			Matrix omega_matrix = pseudoQ_matrix.times(Tc_matrix
					.minus(p_matrix));
			...
			P.set(0, 0, J * x[7] * (b3 * x[1] - b2 * x[2]) + J * x[8]
					* (g3 * x[1] - g2 * x[2]) + J * x[9]
					* (a3 * x[1] - a2 * x[2]));
			P.set(1, 0, J * x[7] * (b1 * x[2] - b3 * x[0]) + J * x[8]
					* (g1 * x[2] - g3 * x[0]) + J * x[9]
					* (a1 * x[2] - a3 * x[0]));
			P.set(2, 0, J * x[7] * (b2 * x[0] - b1 * x[1]) + J * x[8]
					* (g2 * x[0] - g1 * x[1]) + J * x[9]
					* (a2 * x[0] - a1 * x[1]));
	...
	public void print(double t, double[] y) {
		...
		// Matrix q_times_qtrans = new Matrix(q_matrix.times(q_transpose).A,
		// 3,3);
		// Matrix inv_q_times_qtrans = new Matrix(q_times_qtrans.invert().A,
		// 3,3);
		// double[][] pseudoinv_q = q_transpose.times(inv_q_times_qtrans).A;

		double Hb1_wheel = J
				* (Omega1 * b1 + Omega2 * g1 + Omega3 * a1 + Omega4 * d1);
		double Hb2_wheel = J
				* (Omega1 * b2 + Omega2 * g2 + Omega3 * a2 + Omega4 * d2);
		double Hb3_wheel = J
				* (Omega1 * b3 + Omega2 * g3 + Omega3 * a3 + Omega4 * d3);
		...
		double energy = 0.5 * ((Wheelb1 + w1) * (J * Wheelb1 + I1 * w1)
				+ (Wheelb2 + w2) * (J * Wheelb2 + I2 * w2) + (Wheelb3 + w3)
				* (J * Wheelb3 + I3 * w3));
		...
		Matrix inertial_angular_momentum = new Matrix(
				T_transpose.times(angular_momentum_vector));
	...
	double[] convertEulerToTargetQ(double a, double b, double c) {
		...
		double a21 = -Math.cos(phi) * Math.sin(psi) + Math.sin(phi)
				* Math.sin(theta) * Math.cos(psi);
		double a22 = Math.cos(phi) * Math.cos(psi) + Math.sin(phi)
				* Math.sin(theta) * Math.sin(psi);
		double a23 = Math.sin(phi) * Math.cos(theta);
		double a31 = Math.sin(phi) * Math.sin(psi) + Math.cos(phi)
				* Math.sin(theta) * Math.cos(psi);
		double a32 = -Math.sin(phi) * Math.cos(psi) + Math.cos(phi)
				* Math.sin(theta) * Math.sin(psi);
	...
	/**
	 * This method takes pseudo inverse given the appropriate inputs
	 * 
	 * @param inertia
	 * @param w1
	 *            w2 w3
	...
	public static void main(String[] args) {
		...
		float quat_values[][] = new float[5][numberOfPts + 1];
