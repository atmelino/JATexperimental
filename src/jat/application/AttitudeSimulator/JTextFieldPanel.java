 ...
 * And all the components constructed in this class is swing components. The
 * main objective of this class is to create input fields with labels
...
{
	private static final long serialVersionUID = 1810937096232967192L;
	...
	/**
	 * @param direction
	 *            (int)1-> X_AXIS, 2-> Y_AXIS
	 * @param title
	 *            (String) Title of the panel
	 * @param labels
	 *            (String[]) Labels of the corresponding TextFields
	 * @param fields
	 *            (JTextField[]) TextFields to be laid out
	 * @param c
	...
	public JTextFieldPanel(int direction, String title, String labels[],
		...
		if (direction == 1) {
			...
			Dimension columnSize = new Dimension(panelColumnWidth,
					panelColumnHeight);
	 ...
	 * /** Constructor 2
	 * 
	 * @param direction 1-> X_AXIS 2-> Y_AXIS
	 * 
	 ...
	 * 
	 * public JTextFieldPanel(int direction, String title, String labels[],
	 * JTextField fields[], Color c, ImageIcon titleImage) { // Since labels are
	 * supposed to be associated with the text fields, // labels[] and fields[]
	 * are assumed to have the same length. super(); int length = labels.length;
	 * 
	 * // The target of BoxLayout is 'this' JPanel. / if(direction == 1) {
	 * setLayout(new BoxLayout(this, BoxLayout.Y_AXIS)); JPanel rowPanel = new
	 * JPanel(); rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
	 * 
	 * int panelRowWidth = length*150 + length * 5 + 5; int panelRowHeight = 70;
	 * Dimension rowSize = new Dimension (panelRowWidth, panelRowHeight);
	 * Dimension panelSize = new Dimension (panelRowWidth, panelRowHeight + 50);
	 * rowPanel.setMaximumSize(rowSize); rowPanel.setPreferredSize(rowSize);
	 * rowPanel.setMinimumSize(rowSize); rowPanel.setAlignmentX(LEFT_ALIGNMENT);
	 * this.setMaximumSize(panelSize); this.setPreferredSize(panelSize);
	 * this.setMinimumSize(panelSize);
	 * 
	 * rowPanel.setBackground(c); this.setBackground(c);
	 * this.setBorder(BorderFactory.createLineBorder(Color.red));
	 * 
	 * JLabel titleLabel = new JLabel(title); // Associate the font with the
	 * label titleLabel.setFont(titlef); titleLabel.setIcon(titleImage); //
	 ...
	 * this.add(titleLabel);
	 * 
	 * // Add JLabel and JTextField component to this JPanel for(int
	 * i=0;i<length;++i) { JLabel label = new JLabel(labels[i], Label.RIGHT);
	 * rowPanel.add(Box.createHorizontalGlue()); rowPanel.add(label );
	 * rowPanel.add(fields[i]);
	 * 
	 * }
	 * 
	 * this.add(rowPanel); }// End of if else { setLayout(new BoxLayout(this,
	 * BoxLayout.Y_AXIS)); int panelColumnWidth = 25; int panelColumnHeight =
	 * length*70 + length * 5 + 5; Dimension columnSize = new
	 * Dimension(panelColumnWidth, panelColumnHeight);
	 * this.setMaximumSize(columnSize); this.setPreferredSize(columnSize);
	 * this.setMinimumSize(columnSize);
	 * 
	 * 
	 * this.setBackground(c);
	 ...
	 * // Add JLabel and JTextField component to this JPanel for(int
	 * i=0;i<length;++i) { JLabel label = new JLabel(labels[i], Label.RIGHT);
	 * add(Box.createHorizontalGlue()); add(label ); add(fields[i]); }
	 * 
	 * }//End of else
	 * 
	 * 
	...
	public static void main(String[] args) {
		...
		JTextFieldPanel theJPanel = new JTextFieldPanel(2, title, hobby,
				hobbyfields, Color.pink);
		//ResourceLoader c = new ResourceLoader();
		//URL helpURL2 = c.loadURL(theJPanel.getClass(), "images/Domburi7.gif");
		...
		//ImageIcon domburi = new ImageIcon(helpURL2.getFile());
		...
		// theFrame.getContentPane().setLayout(new
		// BoxLayout(theFrame,BoxLayout.Y_AXIS));
		theFrame.getContentPane().add(outerPanel, BorderLayout.CENTER);
		theFrame.setTitle("JTextFieldPanel");
		// theFrame.setSize((int)frameSize.getWidth(),
